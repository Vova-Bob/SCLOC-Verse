...
-- у 4 views, потім DROP тригера + функції + колонки.
--
-- Порядок критичний:
--   1) спершу CREATE OR REPLACE 4 views з виразом (вони перестають залежати від
--      колонки incident_code);
--   2) лише потім DROP TRIGGER + DROP FUNCTION + DROP COLUMN.
...
SELECT (('INC-' || to_char(opened_at, 'YYYY')) || '-') || lpad(id::text, 5, '0') AS incident_id,
       ...
       status, highest_severity, peak_failure_pct, affected_users, affected_installs,
       event_count, owner
FROM public.telemetry_incidents ORDER BY opened_at DESC;
GRANT SELECT ON control_center.incidents TO cc_readonly;

-- =========================================================================
-- 2. Повернути control_center.incident_timeline з виразом
-- =========================================================================
...
FROM public.incident_status_log l JOIN public.telemetry_incidents i ON i.id = l.incident_id
ORDER BY l.changed_at DESC;
GRANT SELECT ON control_center.incident_timeline TO cc_readonly;

-- =========================================================================
-- 3. Повернути control_center.incident_notes_view з виразом
-- =========================================================================
...
SELECT n.id, n.incident_id, n.content, n.created_by, n.created_at,
       (('INC-' || to_char(i.opened_at, 'YYYY')) || '-') || lpad(i.id::text, 5, '0') AS incident_code
FROM public.incident_notes n JOIN public.telemetry_incidents i ON i.id = n.incident_id
ORDER BY n.created_at DESC;
GRANT SELECT ON control_center.incident_notes_view TO cc_readonly;

-- =========================================================================
-- 4. Повернути control_center.notifications з виразом
-- =========================================================================
...
SELECT n.id, n.incident_id, n.notification_type, n.provider, n.status,
       ...
       n.claimed_at, n.claimed_by, n.last_error, n.delivered_at, n.error_message, n.created_at,
       (('INC-' || to_char(i.opened_at, 'YYYY')) || '-') || lpad(i.id::text, 5, '0') AS incident_code,
       i.component, i.signal, i.highest_severity, i.release
FROM public.notification_queue n JOIN public.telemetry_incidents i ON i.id = n.incident_id
ORDER BY n.created_at DESC;
GRANT SELECT ON control_center.notifications TO cc_readonly;

-- =========================================================================
-- 5. DROP тригер + функція + колонка (тепер безпечно — views не посилаються)
-- =========================================================================
DROP TRIGGER IF EXISTS trg_set_incident_code ON public.telemetry_incidents;
DROP FUNCTION IF EXISTS public.set_incident_code();
